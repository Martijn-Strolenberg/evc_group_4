...
class MotorSubscriberNode:
    def __init__(self):
        ...
        self.drive_state = 0          # State machine variable
        self.prev_drive_state = 0     # Previous state variable

        # Initialize state variables
        self.mode_state = 0 # State machine variable for the mode
        ...
        # Initialize sensor reading services
    ...
    def handle_move_straight(self, req):
        ...
        if req.distance > 0: # Check if distance is positive (drive forwards)
            ...
            self.drive_state = 3 # Set the state to drive straight forwards
        ...
        if req.distance < 0: # Check if distance is negative (drive backwards)
            ...
            self.drive_state = 4 # Set the state to drive straight backwards
            return MoveStraightResponse(True)
        return MoveStraightResponse(False)
    ...
    def handle_rotate(self, req):
        ...
        if req.angle > 0: # Check if angle is positive (rotate clockwise)
            ...
            self.drive_state = 1 # Set the state to rotate clockwise
        ...
        if req.angle < 0: # Check if angle is negative (rotate counter-clockwise)
            ...
            self.drive_state = 2 # Set the state to rotate counter-clockwise
            return RotateResponse(True)
        return RotateResponse(False)
    ...
    def handle_const_rotate(self, req):
        ...
        if req.direction > 0: # Check if direction is positive (rotate clockwise) (right?)
            ...
            self.drive_state = 9 # Set the state to constant rotate clockwise
        ...
        if req.direction < 0: # Check if direction is negative (rotate counter-clockwise) (left?)
            ...
            self.drive_state = 8 # Set the state to constant rotate counter-clockwise
            return ConstRotateResponse(True)
        return ConstRotateResponse(False)
    ...
    def handle_const_straight(self, req):
        ...
        if req.direction > 0: # Check if distance is positive (drive forwards)
            ...
            self.drive_state = 6 # Set the state to drive straight forwards
        ...
        if req.direction < 0: # Check if distance is negative (drive backwards)
            ...
            self.drive_state = 7 # Set the state to drive straight backwards
            return ConstStraightResponse(True)
    ...
    def handle_drive_leftwheel(self, req):
        ...
        if req.direction > 0: # Check if distance is positive (drive forwards)
            ...
            self.left_direction = 1
            self.drive_state = 10 # Set the state to drive left wheel forwards
        ...
        if req.direction < 0: # Check if distance is negative (drive backwards)
            ...
            self.left_direction = -1
            self.drive_state = 11 # Set the state to drive left wheel backwards
            return DriveLeftwheelResponse(True)
        return DriveLeftwheelResponse(False)
    ...
    def handle_drive_rightwheel(self, req):
        ...
        if req.direction > 0: # Check if distance is positive (drive forwards)
            ...
            self.right_direction = 1
            self.drive_state = 12 # Set the state to drive right wheel forwards
        ...
        if req.direction < 0: # Check if distance is negative (drive backwards)
            ...
            self.right_direction = -1
            self.drive_state = 13 # Set the state to drive right wheel backwards
            return DriveRightwheelResponse(True)
        return DriveRightwheelResponse(False)
...
# ------------ MODE STATE 0: Driving ---------------------------------------------------------        
        if self.mode_state == 0:
            # === State 0: Command received ===
            if self.drive_drive_state == 0:
                if self.prev_drive_state != self.drive_state:
                    rospy.loginfo("drive_state 0: Waiting for command...")
            ...
            if self.drive_state == 1:
                if self.prev_drive_state != self.drive_state:
                    rospy.loginfo("drive_state 1: Rotate in place clockwise")
                    self.prev_drive_state = self.drive_state
                ...
                if abs(self.angle_diff(self.last_odom_theta, self.start_theta)) >= self.goal_angle: # Check if the robot has rotated the desired angle
                    self.drive_state = 5 # Set the state to stop the robot
                else:
            ...
            if self.drive_state == 2:
                if self.prev_drive_state != self.drive_state:
                    rospy.loginfo("drive_state 2: Rotate in place counter-clockwise")
                    self.prev_drive_state = self.drive_state
                    # update the wheel directions
            ...
            if self.drive_state == 3:
                if self.prev_drive_state != self.drive_state:
                    rospy.loginfo("drive_state 3: Drive straight forwards")
                    self.prev_drive_state = self.drive_state
                    # update the wheel directions
                    self.call_left_wheel_dir(1)  # Set left wheel direction to forward
                    self.call_right_wheel_dir(1) # Set right wheel direction to forward
            ...
            if self.drive_state == 4:
                if self.prev_drive_state != self.drive_state:
                    rospy.loginfo("drive_state 4: Drive straight backwards")
                    self.prev_drive_state = self.drive_state
                    # update the wheel directions
                    self.call_left_wheel_dir(-1)  # Set left wheel direction to backward
                    self.call_right_wheel_dir(-1) # Set right wheel direction to backward
            ...
            if self.drive_state == 5:
                if self.prev_drive_state != self.drive_state:
                    rospy.loginfo("drive_state 5: Stop robot")
                    self.prev_drive_state = self.drive_state
                # Stop the robot
                ...
                if abs(cmd_left) < 1e-2 and abs(cmd_right) < 1e-2:
                    self.drive_state = 0          # Idle, reset state to idle, ready for next command
                    self.cmd_left = 0.0     # make sure the cache is truly zero
                    self.cmd_right = 0.0

            # === State 6: constant drive robot forwards ===
            if self.drive_state == 6:
                if self.prev_drive_state != self.drive_state:
                    rospy.loginfo("drive_state 6: Drive constant Forwards")
                    self.prev_drive_state = self.drive_state
                    # update the wheel directions
                    self.call_left_wheel_dir(1)  # Set left wheel direction to forward
                    self.call_right_wheel_dir(1) # Set right wheel direction to forward
            ...
            if self.drive_state == 7:
                if self.prev_drive_state != self.drive_state:
                    rospy.loginfo("drive_state 7: Drive constant backwards")
                    self.prev_drive_state = self.drive_state
                    # update the wheel directions
                    self.call_left_wheel_dir(-1)  # Set left wheel direction to backward
                    self.call_right_wheel_dir(-1) # Set right wheel direction to backward
            ...
            if self.drive_state == 8:
                if self.prev_drive_state != self.drive_state:
                    rospy.loginfo("drive_state 8: Rotate constant left") # (counter clockwise)
                    self.prev_drive_state = self.drive_state
                    # update the wheel directions
                    self.call_left_wheel_dir(1)  # Set left wheel direction to forward
                    self.call_right_wheel_dir(-1) # Set right wheel direction to backward
            ...
            if self.drive_state == 9:
                if self.prev_drive_state != self.drive_state:
                    rospy.loginfo("drive_state 9: Rotate constant right") # (clockwise)
                    self.prev_drive_state = self.drive_state
                    # update the wheel directions
                    self.call_left_wheel_dir(-1)  # Set left wheel direction to backward
                    self.call_right_wheel_dir(1) # Set right wheel direction to forward
            ...
            if self.drive_state == 10:
                if self.prev_drive_state != self.drive_state:
                    rospy.loginfo("drive_state 10: command left wheel forwards")
                    self.prev_drive_state = self.drive_state
            ...
            if self.drive_state == 11:
                if self.prev_drive_state != self.drive_state:
                    rospy.loginfo("drive_state 11: command left wheel backwards")
                    self.prev_drive_state = self.drive_state
                    # update left wheel direction
            ...
            if self.drive_state == 12:
                if self.prev_drive_state != self.drive_state:
                    rospy.loginfo("drive_state 12: command right wheel forwards")
                    self.prev_drive_state = self.drive_state
            ...
            if self.drive_state == 13:
                if self.prev_drive_state != self.drive_state:
                    rospy.loginfo("drive_state 13: command right wheel backwards")
                    self.prev_drive_state = self.drive_state
                    # update right wheel direction
