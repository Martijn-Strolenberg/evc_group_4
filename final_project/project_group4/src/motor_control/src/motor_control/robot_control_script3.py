...
def call_left_wheel(direction: int, speed: float) -> None:
...
def call_right_wheel(direction: int, speed: float) -> None:
...
def call_const_rotate(direction: int, angular_speed: float) -> None:
...
def call_const_straight(direction: int, speed: float) -> None:
...
def call_move_straight(distance: float, speed: float) -> None:
...
def call_rotate(angle: float, angular_speed: float) -> None:
  ...
  while not rospy.is_shutdown():
    try:
      ...
      if cmd == "left_wheel" and len(parts) == 3:
        call_left_wheel(int(parts[1]), float(parts[2]))
      if cmd == "right_wheel" and len(parts) == 3:
        call_right_wheel(int(parts[1]), float(parts[2]))
      if cmd == "const_rotate" and len(parts) == 3:
        call_const_rotate(int(parts[1]), float(parts[2]))
      if cmd == "const_straight" and len(parts) == 3:
        call_const_straight(int(parts[1]), float(parts[2]))
