...
# from camera.msg import twovids
...
class CameraSubscriberNode:
    ...
    def image_cb(self, data):
        ...
        try:
            ...
            # undis_image = self.resize_with_aspect_ratio(undis_image, 780)
            # make copy 
            cv_image_2 = undis_image.copy()
            # execute OCR function
            cv_image_processed = self.OCR_on_img(cv_image_2,self.confidence)
            # cv_image_processed = self.resize_with_aspect_ratio(cv_image_processed, 780)
...
if __name__ == "__main__":
    # Initialize the node
    rospy.init_node('Motor_OCR_node', anonymous=False, xmlrpc_port=45100, tcpros_port=45101)
