...
class CameraSubscriberNode:
    def __init__(self):
        ...
        self.cmd_rate = 20.0 # generate move commands at 10 Hz
        ...
        self.kp_exp = rospy.get_param("~kp_exp", 1.0)  # Exponent for error scaling, default is 1.0
...
# <============== Image Processing =================>
    def image_cb(self, data):
        ...
        try:
            ...
            thing = cv2.cvtColor(undis_image, cv2.COLOR_BGR2LAB)
            l, a, b = cv2.split(thing)
            clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
            l = clahe.apply(l)
            thing = cv2.merge((l, a, b))
            processed1 = cv2.cvtColor(lab, cv2.COLOR_LAB2BGR)

            hsv = cv2.cvtColor(processed1, cv2.COLOR_BGR2HSV)
            ...
            y23 = int(height * 1 / 2)  # Only consider the bottom 2/3 of the image
            ...
            for cnt in contours:
                ...
                if area > best_area and area < 17000:  # Only consider contours with area < 10000
                    best_area = area
                    best_centroid = (cx, cy)
...
# <=================== Motor control =========================>
    def timer_cb(self, event):
        ...
        if sign == -1:
            velocity_right = np.clip(self.move_vel * error,a_min=-np.inf,a_max = self.turn_vel)
            velocity_left = self.move_vel / error
        else:
            velocity_left = np.clip(self.move_vel * error,a_min=-np.inf,a_max = self.turn_vel)
