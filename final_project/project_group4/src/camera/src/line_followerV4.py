...
class CameraSubscriberNode:
    def __init__(self):
        ...
        # subscriber to receive max speed changes
        self.sub_max_speed_change = rospy.Subscriber(
            "/max_speed",
            Float64,
            self.max_speed_change_cb,
            queue_size = 10
        )
    ...
    def max_speed_change_cb(self,data):
        if not self.initialized:
            return
        if (self.move_vel != data.data):
            self.move_vel = data.data
            rospy.loginfo("Change speed to : {}".format(self.move_vel))     
