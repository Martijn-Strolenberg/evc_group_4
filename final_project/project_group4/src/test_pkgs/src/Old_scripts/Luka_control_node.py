    ...
    def motor_cb(self, data):
        ...
        if not data.blocking:
            ...
            if self.action == 1: # going straight
                ...
                motor.set_wheels_speed(left=(self.gain - self.trim)*self.vel, 
                                       right=(self.gain + self.trim)*self.vel) 
            elif self.action == 2:# going back
                rospy.loginfo("Moving Backward")
                self.vel = -self.vel
                motor.set_wheels_speed(left=(self.gain - self.trim)*self.vel, 
                                       right=(self.gain + self.trim)*self.vel) 
            elif self.action == 3:# going left
                rospy.loginfo("Rotating Left")
                motor.set_wheels_speed(left=-(self.gain - self.trim)*self.vel, 
                                       right=(self.gain + self.trim)*self.vel) 
            elif self.action == 4:# going right
